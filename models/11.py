...
class AutoencoderKL(pl.LightningModule):
    ...
    def forward(self, input, sample_posterior=True, type="sample"):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 1:
            ...
            gt = inputs[0].squeeze(0)
            rec = reconstructions[0].squeeze(0)

            gt = (gt+1)*127.5
            rec = (rec+1)*127.5
            _, _, _, ssim_avg = Structural_Similarity(gt, rec, PIXEL_MAX=255)
            self.log("val/ssim", ssim_avg, sync_dist=self.sync_dist)
    ...
    def test_step(self, batch, batch_idx):
        inputs = batch["image"]
        reconstructions, posterior = self(inputs, type="mode")
